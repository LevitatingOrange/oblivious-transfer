...
use super::super::crypto::{SymmetricCrypt, Digest};
use super::super::util::{read_data, write_data};
    ...
    > SimpleOTSender<T, D, L, S>
{
    ...
    pub async fn send<'a>(&'a mut self, values: &'a[&'a[u8]]) -> Fallible<()> {
        ...
        for (key, value) in keys.into_iter().zip(values) {
            ...
            buf = self.crypt.encrypt(&key, buf)?;
    ...
    > SimpleOTReceiver<T, R, D, L, S>
{
    ...
    pub async fn receive(&mut self, index: usize, n: usize) -> Fallible<Vec<u8>> {
        ...
        let buf = self.crypt.decrypt(&key, buffers.remove(index))?;
        Ok(buf)
    }
}
